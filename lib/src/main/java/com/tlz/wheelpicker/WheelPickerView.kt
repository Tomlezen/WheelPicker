...
//                rectDrawn.height().toFloat(), 0x00f2f2f2, 0xf2f2f2, Shader.TileMode.MIRROR)
  ...
  override fun onDraw(canvas: Canvas?) {
    canvas?.let {
      ...
      (0 until drawnItemCount).map { it + drawnPos }.forEach {
        if (isPosInRang(it)) {
...
//                canvas?.restore()
    ...
    when (event?.action) {
      ...
      MotionEvent.ACTION_MOVE -> {
        if (Math.abs(downPointY - event.y) < touchSlop) {
          ...
          isClickAction = false
          tracker?.addMovement(event)
          var move = (event.y - lastPointY).toInt()
          if (Math.abs(move) > 1) {
            if (if(isMoving) (scrollOffsetY < minFlingY || scrollOffsetY > maxFlingY) else (scrollOffsetY + move < minFlingY || scrollOffsetY + move > maxFlingY)) {
              move /= 3
            }
            scrollOffsetY += move
            lastPointY = event.y.toInt()
      ...
      MotionEvent.ACTION_UP -> {
        ...
        if (isClickAction) {
          ...
          if (Math.abs(velocity) > minimumVelocity) {
            ...
            when {
              scrollOffsetY > maxFlingY -> scroller.startScroll(0, scrollOffsetY, 0, -scrollOffsetY + maxFlingY)
              scrollOffsetY < minFlingY -> scroller.startScroll(0, scrollOffsetY, 0, minFlingY - scrollOffsetY)
              else -> scroller.startScroll(0, scrollOffsetY, 0, computeDistanceToEndPoint(scrollOffsetY % itemHeight))
            }
          }
  ...
  fun setSelectedItemPosition(position: Int) {
    var hPosition = Math.min(position, adapter.itemCount - 1)
    hPosition = Math.max(hPosition, 0)
    selectedItemPosition = hPosition
    currentItemPosition = hPosition
    scrollOffsetY = 0
    computeTextSize()
    computeDrawnCenter()
    computeFlingLimitY()
